...
for db in $databases;
do 
    docker compose exec mariadb mariadb-dump --host=$server --port=$port --user=$user --password=$pass --routines --triggers --databases  $db > $base_folder/$backup_folder/$db.sql
done
